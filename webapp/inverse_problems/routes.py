...
# Get inverse_problems equations and parameters from modular structure
INVERSE_PROBLEMS_EQUATIONS = ALL_PURPOSE_EQUATIONS.get('inverse_problems', {})
INVERSE_PROBLEMS_PARAMETERS = ALL_PURPOSE_PARAMETERS.get('inverse_problems', {})
...
async def inverse_problems_simulate(eq_id: str, request: Request):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{config.API_BASE_URL}/api/inverse_problems/{eq_id}/train",
                json=backend_params,
                timeout=300.0
            )
...
async def inverse_problems_get_results(eq_id: str):
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{config.API_BASE_URL}/api/inverse_problems/{eq_id}/results",
                timeout=30.0
            )
...
async def get_comprehensive_parameters():
    """Get comprehensive parameters for inverse problems"""
    parameters = config.get_parameters_by_purpose("inverse_problems")
    equations = config.get_equations_by_purpose("inverse_problems")
...
async def validate_parameters(request: ComprehensiveTrainingRequest):
    """Validate training parameters for inverse problems"""
    # Basic validation
    if request.epochs <= 0:
    ...
    return {
        "valid": True,
        "message": "Parameters are valid for inverse problems training",
        "purpose": "inverse_problems"
    }
