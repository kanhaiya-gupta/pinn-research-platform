"""
Routes for Uncertainty Analysis PINN Application
"""
...
router = APIRouter(prefix="/purpose/uncertainty", tags=["Uncertainty Analysis"])
...
async def uncertainty_page(request: Request):
    """Uncertainty Analysis main page"""
    ...
    # Get equations that support uncertainty using modular structure
    supported_equations = {}
    for eq_key in UNCERTAINTY_EQUATIONS:
...
async def uncertainty_simulation(request: Request, eq_type: str):
    """Simulation page for uncertainty analysis and specific equation"""
    ...
    if "uncertainty" not in equation_info.get('purposes', []):
        raise HTTPException(status_code=400, detail="Equation does not support uncertainty analysis")
    
    return templates.TemplateResponse(
        "uncertainty/simulation.html",
        {
            "request": request,
            "purpose": purpose_info,
            "purpose_key": "uncertainty",
...
async def uncertainty_results(request: Request, eq_type: str):
    """Results page for uncertainty analysis and specific equation"""
    if eq_type not in config.SUPPORTED_EQUATIONS:
        raise HTTPException(status_code=404, detail="Equation not found")
...
# API endpoints for uncertainty analysis
@router.post("/api/simulate/{eq_type}")
async def uncertainty_simulate(eq_type: str, request: Request):
    """Submit training request for uncertainty analysis"""
    if eq_type not in config.SUPPORTED_EQUATIONS:
        raise HTTPException(status_code=404, detail="Equation not found")
...
async def uncertainty_get_results(eq_type: str):
    """Get results for uncertainty analysis simulation"""
    if eq_type not in config.SUPPORTED_EQUATIONS:
        raise HTTPException(status_code=404, detail="Equation not found")
...
def map_parameters_to_backend(eq_id: str, frontend_params: dict) -> dict:
    """Map frontend parameters to backend format for uncertainty analysis"""
    backend_params = {
    ...
    backend_params["uncertainty_samples"] = frontend_params.get("uncertainty_samples", 1000)  # Number of uncertainty samples
    backend_params["confidence_level"] = frontend_params.get("confidence_level", 0.95)  # Confidence level for intervals
